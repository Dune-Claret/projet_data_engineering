...
def get_graphes(country):
    ...
    fig_sell_price = px.scatter(df_France, x = "certif_UT", y = "price", title = "Price according to sells in " + country, hover_name = "certif_UT", hover_data=['price'], labels = {"certif_UT" : "Selling number", "price" : "Prices"})
    ...
    fig_year = px.histogram(df_year, x = "year", title = "Distribution of top albums by year in " + country, labels = {"year" : "Years", "count" : "Number of top albums"})
    fig_year.update_layout({
        'paper_bgcolor': 'rgb(41,46,48)',
        })
